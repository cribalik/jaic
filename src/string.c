...
static int string_append(String* a, const char* b) {
  int len;
  char* next;
...
static String string_create(char* initial_value) {
  String result;
...
static int string_prepend(String* a, char* b) {
  int len, old_len;
  char* next;
  assert(a->chars.count > 0);
  len = strlen(b);
...
static void string_pop(String* s, int n) {
  assert(s->chars.count > n);
  array_pop_N(&s->chars, n);
  *(char*)array_last(&s->chars) = 0;
}

static char* string_get(String* s) {
  return array_begin(&s->chars);
}

static char* string_clear(String* s) {
  s->chars.count = 1;
  *(char*)array_last(&s->chars) = 0;
  return s->chars.data;
}
...
static void string_append_char(String* s, char c) {
